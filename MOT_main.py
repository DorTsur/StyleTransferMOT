...
parser.add_argument('--levels', type=str, default='43210', help="enc levels string")
...
if args.using_wandb:
    wandb_proj = "mot_barycenter" if not hasattr(args, 'wandb_project_name') else args.wandb_project_name
...
# total number of marginals in the barycenter problem is the number of styles +2 (content, barycenter)
k = args.num_styles+2
if args.bary_method=='gauss':
    mot_b = trainerGauss(args)
else:
    mot_b = trainerMOT(args, k)


def styleTransferMOT(contentImg, styleImgs, saveImg, args, imname):
...
args.folderPath = f'results/num_styles_{args.num_styles}/eps_{args.eps}/num_epochs_{args.epochs}/weights_{args.barycenter_weights}/{datetime_now}'
