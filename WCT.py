...
def styleTransferMOT(contentImg, styleImgs):
    levels = [4]
    newImg = contentImg
    for level in levels:
        ...
        with torch.no_grad():
            encStyles = [mot_b.enc[level](styleImg).detach() for styleImg in styleImgs]
            encContent = mot_b.enc[level](newImg).detach()
...
def styleTransfer(contentImg,styleImg,imname,csF):
    ...
    vutils.save_image(Im1.data.cpu().float(),os.path.join(args.outf,'newrun'+imname))
    return
